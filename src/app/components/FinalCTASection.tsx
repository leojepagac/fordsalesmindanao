  ...
  return (
    <section id="contact" ref={sectionRef} className="relative py-28 md:py-36 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 md:px-12 text-center">
        ...
        <div className="cta-reveal flex flex-col sm:flex-row gap-4 justify-center" style={{ opacity: 0, transform: 'translateY(24px)', transition: 'opacity 0.7s ease, transform 0.7s ease' }}>
          <a href={getFormUrl(CONSULTANT, 'quotation')} target="_blank" rel="noopener noreferrer" className="btn-accent text-base" aria-label="Get Free Quotation">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>
            GET FREE QUOTATION
          </a>
          <a href={getFormUrl(CONSULTANT, 'test-drive')} target="_blank" rel="noopener noreferrer" className="btn-outline-white text-base" aria-label="Book A Test Drive">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polygon points="5 3 19 12 5 21 5 3" /></svg>
            BOOK A TEST DRIVE
          </a>
        </div>
        <div className="cta-reveal mt-12 inline-flex items-center gap-4 glass-card rounded-2xl px-6 py-4" style={{ opacity: 0, transform: 'translateY(24px)', transition: 'opacity 0.7s ease, transform 0.7s ease' }}>
          <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-accent flex-shrink-0">
            <AppImage key={CONSULTANT?.photo} src={CONSULTANT?.photo} alt={`${CONSULTANT?.name} Ford Sales Consultant`} width={48} height={48} className="w-full h-full object-cover" />
          </div>
