...
import { getConsultant, CONSULTANTS, DEFAULT_CONSULTANT } from './ConsultantConfig';
    ...
    <section id="consultant" ref={sectionRef} className="py-20 md:py-28 bg-secondary relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          {/* Left — Photo */}
          <div className="consultant-reveal relative" style={{ opacity: 0, transform: 'translateY(28px)', transition: 'opacity 0.7s ease, transform 0.7s ease' }}>
		...
		src={CONSULTANT?.photo}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.85) 0%, transparent 100%)' }}>
                <p className="text-accent text-xs font-bold uppercase tracking-widest mb-1">Your Ford Consultant</p>
                <h3 className="text-white font-display font-bold text-2xl leading-tight">{CONSULTANT?.name}</h3>
          ...
          <div className="space-y-8">
            ...
            <div className="consultant-reveal" style={{ opacity: 0, transform: 'translateY(28px)', transition: 'opacity 0.7s ease, transform 0.7s ease' }}>
              <p className="text-white/75 text-lg leading-relaxed">
                Helping customers find the perfect Ford vehicle with professional guidance, financing assistance, and test drive scheduling.
              </p>
            ...
            <div className="consultant-reveal" style={{ opacity: 0, transform: 'translateY(28px)', transition: 'opacity 0.7s ease, transform 0.7s ease' }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <a href={`tel:${CONSULTANT?.mobile?.replace(/\s/g, '')}`} className="contact-btn contact-btn-call" aria-label={`Call ${CONSULTANT?.name}`}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" /></svg>
                  Call Now
                </a>
                <a href={CONSULTANT?.messengerLink} target="_blank" rel="noopener noreferrer" className="contact-btn contact-btn-messenger" aria-label={`Message ${CONSULTANT?.name} on Messenger`}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2C6.477 2 2 6.145 2 11.243c0 2.936 1.46 5.558 3.75 7.28V22l3.42-1.875c.914.253 1.883.39 2.83.39 5.523 0 10-4.144 10-9.272S17.523 2 12 2zm1.03 12.49l-2.55-2.72-4.98 2.72 5.48-5.82 2.61 2.72 4.93-2.72-5.49 5.82z" /></svg>
                  Messenger
                </a>
                <a href={CONSULTANT?.viberLink} className="contact-btn contact-btn-viber" aria-label={`Contact ${CONSULTANT?.name} on Viber`}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20.54 3.46A17.2 17.2 0 0 0 12.01 1C5.88 1 .96 5.92.96 12.05c0 2.04.53 4.03 1.55 5.79L1 23l5.28-1.49A11.1 11.1 0 0 0 12 22.96c6.13 0 11.05-4.92 11.05-11.05 0-2.95-1.15-5.73-3.51-7.45zm-8.53 17c-1.63 0-3.23-.44-4.63-1.27l-.33-.2-3.43.97.93-3.43-.22-.35A9.1 9.1 0 0 1 2.92 12C2.92 6.95 7.04 2.96 12 2.96c2.4 0 4.66.93 6.36 2.63A8.97 8.97 0 0 1 21.1 12c0 5.05-4.12 9.04-9.09 9.04zm5.01-6.75c-.27-.14-1.62-.8-1.87-.89-.25-.09-.43-.14-.62.14-.18.27-.72.89-.88 1.08-.16.18-.33.2-.6.07-.27-.14-1.15-.42-2.19-1.35-.81-.72-1.36-1.61-1.52-1.88-.16-.27-.02-.42.12-.55.13-.12.27-.32.41-.48.14-.16.18-.27.27-.45.09-.18.05-.34-.02-.48-.07-.14-.62-1.49-.85-2.04-.22-.54-.45-.46-.62-.47h-.53c-.18 0-.48.07-.73.34-.25.27-.96.94-.96 2.29s.98 2.66 1.12 2.84c.14.18 1.93 2.95 4.68 4.14.65.28 1.16.45 1.56.58.65.21 1.25.18 1.72.11.52-.08 1.62-.66 1.85-1.3.23-.64.23-1.19.16-1.3-.07-.12-.25-.18-.52-.32z" /></svg>
                  Viber
                </a>
                <a href={CONSULTANT?.whatsAppLink} target="_blank" rel="noopener noreferrer" className="contact-btn contact-btn-whatsapp" aria-label={`Contact ${CONSULTANT?.name} on WhatsApp`}>
